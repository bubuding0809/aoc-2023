...
impl HandType {
    fn from(cards: &[String; 5]) -> Self {
        // Look for the card - freq pair with the highest frequency
        let freqs = cards.iter().fold(HashMap::new(), |mut acc, curr| {
            *acc.entry(curr).or_insert(0) += 1;
            acc
        });
        let mut pairs: Vec<(&&String, &i32)> = freqs.iter().collect();
        pairs.sort_by_key(|(_, v)| *v);

        let mut pairs_it = pairs.iter().rev();
        let max_pair = pairs_it.next().unwrap();
        let second_max_pair = pairs_it.next();

        // Coerce J card to match the card with the highest frequency
        let max_count = if *dbg!(max_pair).0 == &"J".to_string() {
            *max_pair.1
                + match second_max_pair {
                    Some(pair) => pair.1,
                    None => &0,
                }
        } else {
            max_pair.1
                + pairs
                    .iter()
                    .find(|(k, _)| k == &&&"J".to_string())
                    .unwrap_or(&(&&"".to_string(), &0_i32))
                    .1
        };

        // Match card count to type of hand
        match dbg!(max_count) {
            ...
            3 => {
                if freqs.iter().any(|(k, v)| *v == 2 && k != max_pair.0) {
...
fn parse_input(input_str: &str) -> Vec<Hand> {
    input_str
        .lines()
        .map(|line| {
...
mod tests {
    ...
    fn shit_works() -> Result<(), Error> {
        ...
        assert_eq!(test_output, "5905");
        Ok(())
    }

    #[test]
    fn test_hand_type() -> Result<(), Error> {
        let test_input = include_str!("./test-input-03.txt");
        ...
        for (hand, ans) in zip(hands, answers) {
            assert_eq!(HandType::from(&hand.cards), ans, "Failed at {:?}", hand);
        }
